...
{
    public function index(Request $request, RealtimeVoiceDiagnosticsReportService $reports): JsonResponse
    {
        $data = $request->validate([
            'days' => [
                ...
                'integer',
                'min:1',
                'max:'.RealtimeVoiceDiagnosticsReportService::MAX_WINDOW_DAYS,
            ],
        ...
        if (isset($data['from'], $data['to'])) {
            ...
            if ($inclusiveDays > RealtimeVoiceDiagnosticsReportService::MAX_WINDOW_DAYS) {
                throw ValidationException::withMessages([
                    'from' => 'The voice quality date window may not exceed '.RealtimeVoiceDiagnosticsReportService::MAX_WINDOW_DAYS.' days.',
                ]);
            }
        } else {
            $days = (int) ($data['days'] ?? RealtimeVoiceDiagnosticsReportService::DEFAULT_WINDOW_DAYS);
