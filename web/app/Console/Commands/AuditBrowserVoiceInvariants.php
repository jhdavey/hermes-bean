...
{
    protected $signature = 'voice:audit-realtime-invariants
        {--json : Emit machine-readable JSON}
        {--chunk=200 : Number of turns or runs read per chunk (1-500)}';

    protected $description = 'Read-only audit of Realtime voice exact-once, terminal, deadline, run, and raw-audio invariants';
    ...
    public function handle(BrowserVoiceInvariantAuditService $audit): int
    {
        ...
        if ((bool) $this->option('json')) {
            ...
            $this->{$passed ? 'info' : 'error'}('Realtime voice invariant audit: '.strtoupper($report['status']));
            $this->line(sprintf(
