...
test('[BV2-USAGE-03] transient usage accounting retries idempotently without disabling voice prematurely', async () => {
    const attempts = [];
    const delays = [];
    const result = await reportVoiceEventReliably({ providerEventId: 'speech:event-1' }, {
    ...
    await assert.rejects(
        () => reportVoiceEventReliably({ providerEventId: 'speech:event-limit' }, {
            send: async () => {
                limitAttempts += 1;
...
test('input transcript deltas stream into one draft and finalize per audio item', () => {
    ...
    transcripts.append({ itemId: 'two', contentIndex: 0, delta: 'discard me' });
    transcripts.append({ itemId: 'two', contentIndex: 2, delta: 'discard me too' });
    transcripts.append({ itemId: 'two:other', contentIndex: 0, delta: 'keep exact sibling' });
    assert.equal(transcripts.discardItem({ itemId: 'two' }), true);
    assert.equal(transcripts.complete({ itemId: 'two', contentIndex: 0 }), '');
    assert.equal(transcripts.complete({ itemId: 'two', contentIndex: 2 }), '');
    assert.equal(transcripts.complete({ itemId: 'two:other', contentIndex: 0 }), 'keep exact sibling');
    assert.equal(transcripts.discardItem({ itemId: 'two' }), false);
});
...
test('evidence-gated transcript prefix handling remains conservative', () => {
...
test('[BV2-DIAGNOSTIC-03] every browser voice transport stage remains explicit and sanitized', () => {
    const clarification = sanitizedVoiceClientFailure(
        Object.assign(new Error('Provider failed. Bearer secret-provider-token'), {
            code: 'clarification failed!',
        }),
        'clarification',
    );
    assert.equal(clarification.stage, 'clarification');
    assert.equal(clarification.code, 'clarification_failed_');
    assert.match(clarification.message, /Bearer \[redacted\]/);
    assert.doesNotMatch(JSON.stringify(clarification), /secret-provider-token/);

    const hostileMessage = 'api_key=provider-secret transcript=Can_you_hear_me pcm=AAAA';
    for (const stage of ['connection', 'transcription']) {
        ...
        const fixedMessage = stage === 'connection'
            ? 'Browser voice connection failed.'
            : 'Browser voice transcription failed.';
        const fixedCode = stage === 'connection'
            ? 'voice_connection_failure'
            : 'voice_transcription_failure';
        assert.equal(diagnostic.stage, stage);
        assert.equal(diagnostic.code, fixedCode);
        assert.equal(diagnostic.message, fixedMessage);
        assert.deepEqual(diagnostic.cause_chain.map(({ code }) => code), [fixedCode, fixedCode]);
        assert.deepEqual(diagnostic.cause_chain.map(({ message }) => message), [fixedMessage, fixedMessage]);
        assert.doesNotMatch(JSON.stringify(diagnostic), /provider-secret|Can_you_hear_me|pcm=|AAAA/);
    }
});

test('[BV2-DIAGNOSTIC-03] reload-scoped failures identify distinct incidents while one incident retry remains idempotent', async () => {
...
test('[BV2-DIAGNOSTIC-03] controlled local worker codes survive content-neutral persistence and hostile codes do not', () => {
...
test('[BV2-DIAGNOSTIC-03] client failure delivery stays pending until success and flushes online or at the next attempt', async () => {
    ...
    const failureId = voiceClientFailureId('transcription', 'item-1');
    const payload = {
    ...
    assert.equal(sent[0].code, 'voice_transcription_failure');
    assert.equal(sent[0].message, 'Browser voice transcription failed.');
    assert.equal(sent[0].cause_chain[0].code, 'voice_transcription_failure');
    assert.equal(sent[0].cause_chain[0].message, 'Browser voice transcription failed.');
...
test('[BV2-DIAGNOSTIC-03] offline client failures survive reload and flush once after connectivity returns', async () => {
    const storage = new MemoryStorage();
    let online = false;
...
test('[BV2-DIAGNOSTIC-03] storage quota failure is observable and enqueue never claims durable safety', () => {
    const notifications = [];
    const reporter = new VoiceClientFailureReporter({
...
test('[BV2-DIAGNOSTIC-03] capacity keeps oldest failures and emits one explicit overflow diagnostic', async () => {
    const storage = new MemoryStorage();
    let online = false;
    const sent = [];
    const reporter = new VoiceClientFailureReporter({
...
test('[BV2-DIAGNOSTIC-03] recreated overflow state cannot collide with a prior durable overflow identity', async () => {
    const storage = new MemoryStorage();
    const overflowIds = [];
...
test('[BV2-DIAGNOSTIC-03] corrupted persisted diagnostics are discarded safely and reported without their content', async () => {
    const storage = new MemoryStorage();
...
test('[BV2-DIAGNOSTIC-03] login scope switching isolates users and explicit deletion clears only the selected scope', () => {
    const storage = new MemoryStorage();
    const reporter = new VoiceClientFailureReporter({
        send: async () => {},
        isOnline: () => false,
...
test('[BV2-DIAGNOSTIC-03] normal logout preserves unsent diagnostics and same-user login flushes them once', async () => {
    const storage = new MemoryStorage();
    let online = false;
    const sent = [];
    const reporter = new VoiceClientFailureReporter({
