...
test('landing Bean reveals the three-step quick tour plus signup and pricing destinations', () => {
    ...
    assert.match(source, /function normalizeLandingDestination\(destination\)/);
    assert.match(source, /const key = normalizeLandingDestination\(action\)/);
    ...
    assert.match(source, /function gracefullyStopVoiceForInlineSignup\(\)/);
    assert.match(source, /const INLINE_SIGNUP_VOICE_STOP_GRACE_MS = 1400/);
    assert.match(source, /gracefullyStopVoiceForInlineSignup\(\)/);
