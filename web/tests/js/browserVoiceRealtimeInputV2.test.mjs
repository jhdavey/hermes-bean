...
test('[BV2-PCM-02] streaming 16 kHz to 24 kHz resampling preserves phase across chunks', () => {
...
test('[BV2-PRIVACY-PCM-01] activation clears provider input before ordered boundary flush', () => {
    const events = [];
    const appends = [];
    const transport = new BrowserVoiceRealtimeInputTransportV2({
        send: (event) => {
            events.push(event);
            return true;
        },
    ...
    assert.equal(BROWSER_VOICE_REALTIME_PCM_RATE, 24000);
});

test('[BV2-PRIVACY-PCM-02] dormant or stale PCM can never cross an inactive generation', () => {
    const events = [];
    const transport = new BrowserVoiceRealtimeInputTransportV2({
        send: (event) => {
            events.push(event);
...
test('[BV2-PCM-FAIL-01] backpressure fails closed without queuing raw audio', () => {
    const events = [];
    const errors = [];
    const transport = new BrowserVoiceRealtimeInputTransportV2({
        send: (event) => {
...
test('[BV2-PCM-FAIL-02] source ordering and clear delivery are terminal fail-closed boundaries', () => {
    const clearFailure = new BrowserVoiceRealtimeInputTransportV2({ send: () => false });
    assert.throws(
