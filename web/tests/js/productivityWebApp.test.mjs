...
test('Bean voice lets ElevenLabs Agent own turn-taking while the client tool calls Bean', () => {
    ...
    assert.match(source, /overrides: firstMessage \? \{/);
    assert.match(source, /agent:\s*\{\n\s*firstMessage,/);
    assert.match(source, /bean_background_original_request/);
    assert.match(source, /bean_background_result: firstMessage/);
    assert.match(source, /startBeanVoiceSession\(\{\n\s*firstMessage: content,/);
    assert.match(source, /closeBeanVoiceForBackgroundWork\('mode_listening_after_handoff'\)/);
