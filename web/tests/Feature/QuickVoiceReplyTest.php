...
class QuickVoiceReplyTest extends TestCase
{
    ...
    public function test_quick_voice_reply_continues_to_agent_for_weather_requests(): void
    {
        Http::fake([
            'https://api.openai.test/v1/chat/completions' => Http::response([
                'id' => 'chatcmpl-weather',
                ...
                'choices' => [[
                    ...
                    'message' => [
                        'role' => 'assistant',
                        'content' => "I'll check Orlando's weather for tomorrow.",
                    ],
                ]],
            ], 200),
        ]);
        ...
        $this->withToken($token)->postJson('/api/assistant/voice/quick-reply', [
            'content' => 'Can you tell me what the weather is in Orlando tomorrow?',
        ])->assertOk()
            ->assertJsonPath('data.text', "I'll check Orlando's weather for tomorrow.")
            ->assertJsonPath('data.model', 'gpt-quick-test')
            ->assertJsonPath('data.continue_agent', true)
            ->assertJsonPath('data.response_contract', 'acknowledged_background');
