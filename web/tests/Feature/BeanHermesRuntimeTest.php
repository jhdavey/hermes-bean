...
PHP);
    ...
    public function test_calendar_event_create_interprets_naive_datetimes_in_the_user_timezone(): void
    {
        Carbon::setTestNow(Carbon::parse('2026-07-22T19:00:00Z'));
        $this->apiToken('bean-calendar-create-local-time@example.com');
        $user = User::where('email', 'bean-calendar-create-local-time@example.com')->firstOrFail();
        $session = app(BeanRuntimeService::class)->createSession($user, null, 'America/New_York');
        $run = BeanRun::create([
            'bean_session_id' => $session->id,
            'user_id' => $user->id,
            ...
            'status' => 'running',
            'mode' => 'hermes',
            'input' => 'create event at local time',
            'metadata' => ['time_context' => app(\App\Services\Bean\BeanTimeContext::class)->forSession($session)],
            'started_at' => now(),
        ]);
        ...
        $result = app(BeanDashboardToolBridgeService::class)->execute($this->signedToolContextForTimezone($user, $session, $run, 'America/New_York'), [
            ...
            'arguments' => [
                'title' => 'Local breakfast meeting',
                'starts_at' => '2026-07-24 09:30',
                'ends_at' => '2026-07-24 10:45',
            ],
        ]);

        $this->assertTrue((bool) ($result['ok'] ?? false));
        $event = CalendarEvent::where('user_id', $user->id)->where('title', 'Local breakfast meeting')->firstOrFail();
        $this->assertSame('2026-07-24T13:30:00+00:00', $event->starts_at->toIso8601String());
        $this->assertSame('2026-07-24T14:45:00+00:00', $event->ends_at->toIso8601String());
        $this->assertSame('2026-07-24T09:30:00-04:00', $result['item']['starts_at_local']);
