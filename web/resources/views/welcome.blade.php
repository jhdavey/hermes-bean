...
<head>
    ...
    <style>
        ...
        .bean-proof-screen{position:absolute;left:4.8%;right:4.8%;top:6.7%;bottom:9.4%;border-radius:25px;background:linear-gradient(180deg,#fbfef9,#f6fbf5);box-shadow:inset 0 0 0 1px rgba(221,233,221,.92);padding:16px 14px 58px;opacity:0;transform:translateX(26px);display:grid;grid-template-rows:auto auto 1fr;gap:12px;overflow:hidden}
        .bean-proof-top{display:flex;align-items:center;justify-content:space-between;gap:10px}.bean-proof-top strong{font-size:17px;letter-spacing:-.03em}.bean-proof-pill{min-height:25px;display:inline-flex;align-items:center;border-radius:999px;background:#e9f8ed;color:#176b32;padding:0 10px;font-size:10px;font-weight:800}.bean-proof-week{display:grid;grid-template-columns:repeat(5,1fr);gap:6px}.bean-proof-day{display:grid;place-items:center;gap:2px;min-height:48px;border-radius:14px;background:#fff;border:1px solid #e7eee7;color:#647067;font-size:9px;font-weight:800}.bean-proof-day b{display:grid;place-items:center;width:22px;height:22px;border-radius:999px;color:#1f2d24;font-size:11px}.bean-proof-day.active{background:#e9f8ed;border-color:#bce8c8;color:#176b32}.bean-proof-day.active b{background:#7bc98c;color:#173a28}.bean-proof-event,.bean-proof-reminder{display:grid;grid-template-columns:auto 1fr;gap:10px;align-items:start;border:1px solid #e6eee6;background:#fff;border-radius:18px;padding:12px;box-shadow:0 8px 18px rgba(21,45,28,.06)}.bean-proof-time{display:grid;place-items:center;min-width:54px;min-height:42px;border-radius:14px;background:#f1f8f3;color:#176b32;font-size:11px;font-weight:900}.bean-proof-event b,.bean-proof-reminder b{display:block;font-size:13px;line-height:1.15}.bean-proof-event span,.bean-proof-reminder span{display:block;margin-top:4px;color:#6c766d;font-size:10.5px;font-weight:700}.bean-proof-reminder{grid-template-columns:auto 1fr;margin-top:12px}.bean-proof-check{display:grid;place-items:center;width:24px;height:24px;border-radius:999px;background:#dcfce7;color:#12893a;font-size:13px;font-weight:900}.bean-proof-nav{position:absolute;left:4.8%;right:4.8%;bottom:1.9%;height:52px;border-radius:0 0 24px 24px;background:rgba(255,255,255,.94);border-top:1px solid #e5ede5;display:grid;grid-template-columns:repeat(4,1fr);align-items:center}.bean-proof-nav span{display:grid;place-items:center;gap:3px;color:#7a847d;font-size:8px;font-weight:800}.bean-proof-nav i{width:20px;height:20px;border-radius:9px;border:2px solid currentColor;display:block}.bean-proof-nav .calendar i{border-radius:6px}.bean-proof-nav .reminders i{border-radius:999px}.bean-proof-nav .active{color:#16a34a}.bean-demo-tap{position:absolute;z-index:7;width:32px;height:32px;border-radius:999px;background:rgba(22,163,74,.28);border:2px solid rgba(22,163,74,.9);box-shadow:0 0 0 0 rgba(22,163,74,.22);opacity:0;transform:scale(.64)}.bean-demo-tap:after{content:"";position:absolute;inset:8px;border-radius:inherit;background:#16a34a}.bean-demo-tap.calendar{left:39%;bottom:4.6%}.bean-demo-tap.reminders{left:63%;bottom:4.6%}.bean-demo-overlay:has(.bean-demo-proof-shell) .bean-demo-soft-mask{animation:beanSoftMaskProofCycle 16s linear infinite both}.bean-demo-overlay:has(.bean-demo-proof-shell) .bean-demo-input:before{animation:beanInputPlaceholderProofCycle 16s linear infinite both}.bean-demo-overlay:has(.bean-demo-proof-shell) .bean-demo-input-text{animation:beanInputTextProofCycle 16s linear infinite both}.bean-demo-overlay:has(.bean-demo-proof-shell) .bean-demo-user{animation:beanCardUserProofCycle 16s ease infinite both}.bean-demo-overlay:has(.bean-demo-proof-shell) .bean-demo-progress{animation:beanCardProgressProofCycle 16s ease infinite both}.bean-demo-overlay:has(.bean-demo-proof-shell) .bean-demo-result{animation:beanCardResultProofCycle 16s ease infinite both}.bean-proof-screen.calendar{animation:beanCalendarScreenCycle 16s ease infinite both}.bean-proof-screen.reminders{animation:beanReminderScreenCycle 16s ease infinite both}.bean-demo-tap.calendar{animation:beanTapCalendarCycle 16s ease infinite both}.bean-demo-tap.reminders{animation:beanTapReminderCycle 16s ease infinite both}@keyframes beanSoftMaskProofCycle{0%,68%,100%{opacity:1}69%,95%{opacity:0}}@keyframes beanInputPlaceholderProofCycle{0%,3%,33%,100%{opacity:1}4%,32%{opacity:0}}@keyframes beanInputTextProofCycle{0%,4%{opacity:0;clip-path:inset(0 100% 0 0)}5%{opacity:1;clip-path:inset(0 100% 0 0)}24%,31%{opacity:1;clip-path:inset(0 0 0 0)}32%,100%{opacity:0;clip-path:inset(0 0 0 0)}}@keyframes beanCardUserProofCycle{0%,31%,96%,100%{opacity:0;transform:translateY(8px)}34%,62%{opacity:1;transform:translateY(0)}}@keyframes beanCardProgressProofCycle{0%,39%,52%,100%{opacity:0;transform:translateY(8px)}42%,49%{opacity:1;transform:translateY(0)}}@keyframes beanCardResultProofCycle{0%,50%,67%,100%{opacity:0;transform:translateY(8px)}53%,64%{opacity:1;transform:translateY(0)}}@keyframes beanTapCalendarCycle{0%,61.5%,68%,100%{opacity:0;transform:scale(.64);box-shadow:0 0 0 0 rgba(22,163,74,.22)}62.5%,66%{opacity:1;transform:scale(1);box-shadow:0 0 0 12px rgba(22,163,74,.08)}64%{opacity:.95;transform:scale(.78);box-shadow:0 0 0 20px rgba(22,163,74,0)}}@keyframes beanCalendarScreenCycle{0%,66%,82%,100%{opacity:0;transform:translateX(26px)}68%,78%{opacity:1;transform:translateX(0)}}@keyframes beanTapReminderCycle{0%,77.5%,84%,100%{opacity:0;transform:scale(.64);box-shadow:0 0 0 0 rgba(22,163,74,.22)}78.5%,82%{opacity:1;transform:scale(1);box-shadow:0 0 0 12px rgba(22,163,74,.08)}80%{opacity:.95;transform:scale(.78);box-shadow:0 0 0 20px rgba(22,163,74,0)}}@keyframes beanReminderScreenCycle{0%,82%,98%,100%{opacity:0;transform:translateX(26px)}84%,95%{opacity:1;transform:translateX(0)}}@media(prefers-reduced-motion:reduce){.bean-demo-overlay:has(.bean-demo-proof-shell) .bean-demo-soft-mask,.bean-proof-screen.reminders{opacity:1!important;transform:none!important}.bean-proof-screen.calendar,.bean-demo-tap{display:none!important}}
    </style>
</head>
<body>
                            ...
                            <div class="bean-demo-overlay">
                                ...
                                <div class="bean-demo-proof-shell" aria-hidden="true">
                                    <div class="bean-proof-screen calendar">
                                        <div class="bean-proof-top"><strong>Calendar</strong><span class="bean-proof-pill">Friday</span></div>
                                        <div class="bean-proof-week">
                                            <span class="bean-proof-day">Mon <b>15</b></span>
                                            <span class="bean-proof-day">Tue <b>16</b></span>
                                            <span class="bean-proof-day">Wed <b>17</b></span>
                                            <span class="bean-proof-day">Thu <b>18</b></span>
                                            <span class="bean-proof-day active">Fri <b>19</b></span>
                                        </div>
                                        <div class="bean-proof-event"><span class="bean-proof-time">7:00<br>PM</span><div><b>Dinner with Lauren</b><span>Friday · household calendar</span></div></div>
                                        <div class="bean-proof-nav"><span><i></i>Home</span><span class="calendar active"><i></i>Calendar</span><span class="reminders"><i></i>Reminders</span><span><i></i>Tasks</span></div>
                                    </div>
                                    <div class="bean-proof-screen reminders">
                                        <div class="bean-proof-top"><strong>Reminders</strong><span class="bean-proof-pill">Before dinner</span></div>
                                        <div class="bean-proof-reminder"><span class="bean-proof-check">✓</span><div><b>Bring the gift</b><span>Friday · 6:00 PM</span></div></div>
                                        <div class="bean-proof-event"><span class="bean-proof-time">7:00<br>PM</span><div><b>Dinner with Lauren</b><span>Calendar event is ready</span></div></div>
                                        <div class="bean-proof-nav"><span><i></i>Home</span><span class="calendar"><i></i>Calendar</span><span class="reminders active"><i></i>Reminders</span><span><i></i>Tasks</span></div>
                                    </div>
                                    <span class="bean-demo-tap calendar" aria-hidden="true"></span>
                                    <span class="bean-demo-tap reminders" aria-hidden="true"></span>
                                </div>
                            </div>
                        </div>
