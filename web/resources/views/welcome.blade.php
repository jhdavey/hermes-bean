...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;min-height:100vh;background:radial-gradient(circle at 12% 4%,rgba(246,212,107,.34),transparent 28rem),radial-gradient(circle at 88% 8%,rgba(23,182,95,.18),transparent 28rem),linear-gradient(135deg,#fffaf0 0%,#edf8ed 52%,#f8fbf2 100%);font-family:'Instrument Sans',ui-sans-serif,system-ui,sans-serif;color:var(--ink)}
        ...
        @media(max-width:860px){.nav-links a:not(.nav-cta){display:none}.hero,.signup{grid-template-columns:1fr}.hero{padding-top:28px}.cards{grid-template-columns:1fr}h1{font-size:clamp(3rem,15vw,5rem)}.phone{border-radius:28px}.timeline{grid-template-columns:42px 1fr 1fr}.week{padding-inline:10px}.shell{width:min(100% - 24px,1120px)}}
    </style>
</head>
<body>
    <header class="shell nav">
        ...
        <nav class="nav-links" aria-label="Primary navigation">
            ...
            <a href="#early-access" class="nav-cta">Join beta</a>
        </nav>
    </header>
    ...
    <main class="shell">
        <section class="hero">
            <div>
                ...
                <div class="hero-actions">
                    <a class="button primary" href="#early-access">Join the beta</a>
                    <a class="button secondary" href="#how-it-works">See what Bean can do</a>
                </div>
        ...
        <section class="signup" id="early-access">
            <div>
                <h2>Get early access to the agent you can actually use.</h2>
                <p>We are inviting early users who want Bean to become a practical daily assistant — not a toy demo. Tell us how you would use it and we will prioritize beta invites around real workflows.</p>
                <ul>
            ...
            <form class="form" action="{{ route('early-access.store') }}" method="post">
                @csrf
                @if (session('early_access_status'))
                    <div class="status" role="status">{{ session('early_access_status') }}</div>
                @endif
                @if ($errors->any())
                    <div class="errors" role="alert">Please check your email address and try again.</div>
                @endif
                <div class="field">
                    <label for="name">Name</label>
                    <input id="name" name="name" autocomplete="name" value="{{ old('name') }}" placeholder="Your name">
                </div>
                <div class="field">
                ...
                <div class="field">
                    <label for="use_case">What should Bean help you run?</label>
                    <textarea id="use_case" name="use_case" placeholder="Calendar planning, reminders, family tasks, email approvals, weekly routines...">{{ old('use_case') }}</textarea>
                </div>
