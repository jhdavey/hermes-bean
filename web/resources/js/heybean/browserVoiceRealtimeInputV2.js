export const BROWSER_VOICE_LOCAL_PCM_RATE = 16000;
export const BROWSER_VOICE_REALTIME_PCM_RATE = 24000;
export const BROWSER_VOICE_REALTIME_MAX_BUFFERED_BYTES = 256 * 1024;
...
export class StreamingPcm16Resampler {
    constructor({
        inputSampleRate = BROWSER_VOICE_LOCAL_PCM_RATE,
        outputSampleRate = BROWSER_VOICE_REALTIME_PCM_RATE,
    } = {}) {
...
export class BrowserVoiceRealtimeInputTransportV2 {
    constructor({
        send,
        bufferedAmount = () => 0,
        ...
        onAppend = () => {},
        onError = () => {},
        maxBufferedBytes = BROWSER_VOICE_REALTIME_MAX_BUFFERED_BYTES,
    } = {}) {
    ...
    activate({ generation } = {}) {
        ...
        if (this.send({ type: 'input_audio_buffer.clear' }) !== true) {
            return this.#fail('Realtime transcription was not ready to clear its input buffer.');
        }
    ...
    append({ generation, sourceSequence, sampleRate, samples, released = false } = {}) {
        ...
        if (Number(sampleRate) !== BROWSER_VOICE_LOCAL_PCM_RATE) {
        ...
        if (pendingBytes > this.maxBufferedBytes) {
            return this.#fail('Realtime transcription could not keep up with microphone audio.');
        }
        ...
        if (this.send({ type: 'input_audio_buffer.append', audio }) !== true) {
            return this.#fail('Realtime transcription disconnected while receiving microphone audio.');
        }
        try {
