...
export async function reportVoiceEventReliably(report, {
    send,
    retryDelaysMs = [250, 750, 1500],
    delay = (milliseconds) => new Promise((resolve) => globalThis.setTimeout(resolve, milliseconds)),
    shouldRetry = () => true,
    ...
    if (typeof send !== 'function') throw new TypeError('Voice event reporting requires a sender.');
    const schedule = Array.isArray(retryDelaysMs) ? retryDelaysMs : [];
    let attempt = 0;
    while (true) {
        if (!shouldContinue()) {
            throw Object.assign(new Error('Voice event reporting was superseded.'), {
                code: 'voice_event_reporting_superseded',
            });
        }
        try {
            return await send(report, attempt);
        } catch (error) {
            if (attempt >= schedule.length || !shouldRetry(error, attempt)) throw error;
...
const PERSISTED_FAILURE_MESSAGES = Object.freeze({
    ...
    transcription: CONTENT_NEUTRAL_FAILURE_MESSAGES.transcription,
...
const PERSISTED_FAILURE_CODES = Object.freeze({
    ...
    transcription: CONTENT_NEUTRAL_FAILURE_CODES.transcription,
...
export function sanitizedVoiceClientFailure(error, stage = 'local_wake') {
    ...
    const fallback = normalizedStage === 'startup'
        ...
        : normalizedStage === 'clarification'
            ? ['voice_clarification_failure', 'Browser voice clarification failed.']
        : normalizedStage === 'connection'
            ? ['voice_connection_failure', 'Browser voice connection failed.']
        : normalizedStage === 'transcription'
            ? ['voice_transcription_failure', 'Browser voice transcription failed.']
...
export class VoiceClientFailureReporter {
    ...
    async flush() {
        ...
        this.inFlight = (async () => {
            try {
                await reportVoiceEventReliably(payload, {
                    send: this.send,
...
export function stripRealtimeLocalWakePrefix(text, { wakeConfirmed = false } = {}) {
    ...
    return transcript
        .replace(/^\s*(?:(?:hey|they|he)[\s,.-]+(?:bean|ben|bin|bing|being|beane)|habe(?:en|ing)|bean)\b[\s,.:;!?-]*/i, '')
        .trim();
}
...
export function isRealtimeWakeAddressOnly(text, { wakeConfirmed = false } = {}) {
    ...
    return stripRealtimeLocalWakePrefix(transcript, { wakeConfirmed: true }) === ''
        && /^(?:(?:hey|they|he)[\s,.-]+(?:bean|ben|bin|bing|being|beane)|habe(?:en|ing)|bean)\b[\s.!?,-]*$/i.test(transcript);
}
...
export class RealtimeInputTranscriptBuffer {
    ...
    append({ itemId = '', contentIndex = 0, delta = '' } = {}) {
        const identity = this.#identity(itemId, contentIndex);
        if (!identity) return '';
        const [id, index] = identity;
        const itemParts = this.parts.get(id) || new Map();
        const next = `${itemParts.get(index) || ''}${String(delta || '')}`;
        itemParts.set(index, next);
        this.parts.set(id, itemParts);
        return next;
    }

    complete({ itemId = '', contentIndex = 0, transcript = '' } = {}) {
        const identity = this.#identity(itemId, contentIndex);
        if (!identity) return String(transcript || '').trim();
        const [id, index] = identity;
        const itemParts = this.parts.get(id);
        const buffered = itemParts?.get(index) || '';
        itemParts?.delete(index);
        if (itemParts && itemParts.size === 0) this.parts.delete(id);
