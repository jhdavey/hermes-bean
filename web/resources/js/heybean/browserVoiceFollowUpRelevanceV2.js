 ...
 */
export function classifyBrowserVoiceFollowUpRelevance(text, { final = false, responseExpected = false } = {}) {
    ...
    const answerResponse = /^(?:yes|yep|no|nope|okay|ok|sure|please)$/;
    const standaloneControl = /^(?:thanks|thank you|that's all|that is all|goodbye|bye|take care|stop|quiet|wait|repeat that|say that again)$/;
    const directAction = /^(?:(?:also|and|then|actually|instead|please)\s+)*(?:tell|show|check|find|look up|read|create|add|make|set|schedule|remind|delete|remove|change|move|reschedule|cancel|complete|mark|plan|draft|write|save|send|call|text|help|repeat|say|put|give|open|update|edit|rename|start|finish)\b/;
    ...
    const assistantQuestion = /^(?:can|could|would|will|should)\s+you\s+(?:tell|show|check|find|look|read|create|add|make|set|schedule|remind|delete|remove|change|move|cancel|plan|draft|write|save|repeat|say|help|hear|put|give|open|update|edit|rename|start|finish)\b/;
    ...
    const firstPersonRequest = /^(?:now\s+)?i\s+(?:need|want|wanna|would like|would love|meant|asked|said|could use|have to)\b/;
    const contextualContinuation = /^(?:(?:and|also|now)\s+)?(?:what|how)\s+about\s+(?:today|tomorrow|tonight|later|the\s+(?:first|second|next|last)\s+one)\b|^(?:and|also|now)\s+(?:the\s+|a\s+)?(?:time|date|weather|calendar|schedule|reminder|task|event|note|first|second|next|last)\b|^(?:a|the)\s+(?:reminder|task|event|note)\b|^(?:the\s+)?(?:first|second|next|last)\s+one\b|^(?:today|tomorrow|tonight|later)(?:\s+(?:at|around|after|before)\s+(?:noon|midnight|\d{1,2}(?::\d{2})?\s*(?:a\s*m|p\s*m)?))?\??$|^(?:at|around|after|before)\s+(?:noon|midnight|\d{1,2}(?::\d{2})?\s*(?:a\s*m|p\s*m)?)\??$/;
    const explicitCorrection = /^(?:no\s+)?(?:wait|actually|instead|correction)\b|^(?:make|change|move|set)\s+(?:it|that)\b/;
    // Admit structurally addressed imperatives without attempting to enumerate
    // every verb Bean may ever support. Requiring the object pronoun directly
    // after the leading verb keeps ordinary room statements fail-closed while
    // allowing natural forms such as "put that...", "email me...", and
    // "open my..." to reach the authoritative server router.
    const addressedImperative = /^(?:please\s+)?[a-z][a-z'-]*\s+(?:me|my|mine|that|this|it|those|these)\b/;
    const politeRequest = /^please\s+[a-z][a-z'-]*(?:\s+\S+)+$/;

    if ((responseExpected && answerResponse.test(directAddress))
        || standaloneControl.test(directAddress)
        ...
        || addressedImperative.test(directAddress)
        || politeRequest.test(directAddress)) {
        return BROWSER_VOICE_FOLLOW_UP_RELEVANCE.MEANINGFUL;
    }
