...
class Stream {
  ...
  acceptWaveform(sampleRate, samples) {
    if (!this.handle) {
      throw new Error('Cannot accept audio on a freed keyword stream');
    }
  ...
  inputFinished() {
    if (!this.handle) {
      throw new Error('Cannot finish a freed keyword stream');
    }
...
class Kws {
  constructor(configObj, Module) {
    ...
    if (!handle) {
      throw new Error('Failed to create keyword spotter');
    }
  ...
  createStream(keywords = '') {
    if (!this.handle) {
      throw new Error('Cannot create a stream from a freed keyword spotter');
    }

    let handle;
    if (keywords) {
      const length = this.Module.lengthBytesUTF8(keywords) + 1;
      const pointer = this.Module._malloc(length);
      if (!pointer) {
        throw new Error('Failed to allocate custom keyword buffer');
      }

      try {
        this.Module.stringToUTF8(keywords, pointer, length);
        handle = this.Module._SherpaOnnxCreateKeywordStreamWithKeywords(
            this.handle, pointer);
      } finally {
        this.Module._free(pointer);
      }
    } else {
      handle = this.Module._SherpaOnnxCreateKeywordStream(this.handle);
    }

    if (!handle) {
      throw new Error('Failed to create keyword stream');
    }
  ...
  decodeMultiple(streams) {
    if (!Array.isArray(streams) || streams.length === 0) {
      return;
    }

    if (!this.handle) {
      throw new Error('Cannot decode streams with a freed keyword spotter');
    }

    for (const stream of streams) {
      if (!stream || !stream.handle) {
        throw new Error('Cannot decode a freed keyword stream');
      }
    }

    const pointer = this.Module._malloc(streams.length * 4);
    if (!pointer) {
      throw new Error('Failed to allocate keyword stream pointer array');
    }

    try {
      for (let i = 0; i < streams.length; ++i) {
        this.Module.setValue(pointer + i * 4, streams[i].handle, 'i8*');
      }
      this.Module._SherpaOnnxDecodeMultipleKeywordStreams(
          this.handle, pointer, streams.length);
    } finally {
      this.Module._free(pointer);
    }
  }
...
function createKws(Module, myConfig) {
  ...
  let configObj = {
    ...
    keywords: 'HH EY1 B IY1 N :1 #0.1 @HEY_BEAN'
  };
