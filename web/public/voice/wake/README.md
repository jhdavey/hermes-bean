...
- Construct a classic worker from `/voice/wake/wake-worker.js?generation=<non-negative integer>`. The worker initializes automatically. Do not open the gate before its matching `ready` message.
- The worklet accepts `{type: "open"|"close"|"destroy", generation}`. It posts transferable `{type: "audio", samples: Float32Array, sequence, generation}` chunks and `{type: "error", ...}`.
- The provider output has a 320 ms local delay. While closed, that delay line is filled but the output remains bit-exact zero; opening begins with the buffered command onset so wake detection cannot clip the first word. The delay is reset at each generation boundary.
