...
- The July 13 v12 generic-verifier replay passed 119/120 Bean Voice QA journeys (99.17% against a 95% requirement) independently in Playwright Chromium, installed Google Chrome, and Playwright WebKit. Every engine recorded 24/24 isolated strict wakes, 6/6 `Hey Bean` plus command releases, 100% ongoing-speech strict wake, 23/24 missed-Hey address journeys, 0/54 false activations across nine privacy families (including `pretty girl` and `pretty good` as held-out QA inputs only), 6/6 reset recoveries, zero pre-confirmation PCM, complete activated-PCM handoff, and no runtime error. Wake p95 was 424.9 ms in Chromium, 427.8 ms in installed Chrome, and 479 ms in WebKit. Runtime has one `HEY_BEAN` timing candidate, one generic verifier operating point, and no incident-specific negative aliases. This is deterministic prerecorded evidence, not representative physical-microphone release certification.
