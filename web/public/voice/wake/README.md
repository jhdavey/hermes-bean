...
- A strict `Hey Bean` proposal is compatible with either learned addressed positive class at the strict proposal threshold; it always preserves strict activation and the strict safe boundary. An address-only proposal is compatible only with `missed_hey_confirmation` at the address threshold. A compatible classifier positive and a KWS proposal are both required. Deterministic code then uses that proposal's timestamp to establish the corresponding safe release boundary. A rejected or stale proposal returns silently to wake-only and erases retained PCM.
