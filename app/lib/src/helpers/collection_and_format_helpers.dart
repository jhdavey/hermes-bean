...
List<BeanTask> _replaceTask(List<BeanTask> tasks, BeanTask replacement) => tasks
    .map((task) => task.id == replacement.id ? replacement : task)
    .toList(growable: false);

List<BeanTask> _removeTask(List<BeanTask> tasks, int taskId) =>
    tasks.where((task) => task.id != taskId).toList(growable: false);

List<BeanTask> _mergeTaskLists(
  List<BeanTask> primary,
  List<BeanTask> secondary,
) {
  final byId = <int, BeanTask>{};
...
List<BeanNote> _sortedNotes(List<BeanNote> notes) {
  final sorted = [...notes];
  sorted.sort((a, b) {
...
List<BeanNoteFolder> _sortedNoteFolders(List<BeanNoteFolder> folders) {
...
List<BeanNoteFolder> _upsertNoteFolder(
  List<BeanNoteFolder> folders,
  BeanNoteFolder folder,
) {
  final next = [...folders];
...
List<BeanNote> _upsertNote(List<BeanNote> notes, BeanNote note) {
  final next = [...notes];
...
bool _taskIsSubtask(BeanTask task) => task.parentTaskId != null;

List<BeanTask> _subtasksFor(BeanTask task, List<BeanTask> tasks) {
...
List<BeanTask> _tasksForTodayAgenda(List<BeanTask> tasks, DateTime day) {
...
List<BeanTask> _tasksForMonthAgenda(List<BeanTask> tasks, DateTime month) {
...
int? _taskDaysAway(BeanTask task) {
  final dueAt = _parseTaskDueDate(task);
...
int? _reminderDaysAway(BeanReminder reminder) {
...
int _compareTasksByCompletionAndDueDate(BeanTask a, BeanTask b) {
...
int _compareRemindersByCompletionAndDueDate(BeanReminder a, BeanReminder b) {
...
List<BeanTask> _criticalTasksForToday(List<BeanTask> tasks) {
  final today = _dateOnly(DateTime.now());
  final visible = tasks.where((task) {
...
List<BeanCalendarEvent> _criticalEventsForToday(
...
List<BeanReminder> _criticalRemindersForToday(List<BeanReminder> reminders) {
  final today = _dateOnly(DateTime.now());
...
bool _taskIsCritical(BeanTask task) => task.isCritical || _taskIsOverdue(task);

bool _reminderIsCritical(BeanReminder reminder) =>
    reminder.isCritical || _reminderIsOverdue(reminder);

bool _taskIsOverdue(BeanTask task) {
  if (_taskIsCompleted(task)) return false;
  final dueAt = _parseTaskDueDate(task);
  return dueAt != null && dueAt.isBefore(DateTime.now());
}

bool _reminderIsOverdue(BeanReminder reminder) {
  if (_reminderIsCompleted(reminder)) return false;
  final dueAt = _parseReminderDueDate(reminder);
  return dueAt != null && dueAt.isBefore(DateTime.now());
}

bool _taskIsCompleted(BeanTask task) {
  return task.status == 'completed';
}

bool _reminderIsCompleted(BeanReminder reminder) {
  return reminder.status == 'completed';
}

String _taskSubtitle(BeanTask task) {
...
String _reminderSubtitle(BeanReminder reminder) {
...
DateTime? _parseReminderDueDate(BeanReminder reminder) {
...
bool _eventIsAllDay(BeanCalendarEvent event) {
  return event.metadata?['all_day'] == true;
}

bool _eventRendersAboveTimeline(BeanCalendarEvent event) =>
    _eventIsAllDay(event) || _eventIsTimedMultiDay(event);

bool _eventIsTimedMultiDay(BeanCalendarEvent event) =>
    !_eventIsAllDay(event) && _eventSpansMultipleDays(event);

bool _eventSpansMultipleDays(BeanCalendarEvent event) {
...
DateTime? _multiDayEventStartDay(BeanCalendarEvent event) {
  final start = _parseCalendarEventDateTime(event.startsAt);
  return start == null ? null : _dateOnly(start);
}

DateTime? _multiDayEventEndDay(BeanCalendarEvent event) {
  final start = _parseCalendarEventDateTime(event.startsAt);
  final end = _parseCalendarEventDateTime(event.endsAt);
...
String _eventTimeRangeShort(BeanCalendarEvent event) {
  final start = _parseCalendarEventDateTime(event.startsAt);
  final end = _parseCalendarEventDateTime(event.endsAt);
...
bool _taskIsRecurring(BeanTask task) {
...
DateTime? _parseTaskDueDate(BeanTask task) {
...
List<_CommandCenterAgendaItem> _commandCenterAgendaItems({
  required List<BeanTask> tasks,
  required List<BeanReminder> reminders,
  required List<BeanCalendarEvent> calendar,
}) {
...
List<_CommandCenterGlanceDay> _commandCenterGlanceDays(
  List<BeanCalendarEvent> calendar,
) {
  final today = _dateOnly(DateTime.now());
  final days = <DateTime>[
...
List<BeanCalendarEvent> _commandCenterEventsForGlanceDay(
  List<BeanCalendarEvent> calendar,
  DateTime day,
) {
...
DateTime _commandCenterGlanceSortTime(BeanCalendarEvent event, DateTime day) {
  final start = _parseCalendarEventDateTime(event.startsAt);
...
String _commandCenterGlanceEventTime(BeanCalendarEvent event) {
