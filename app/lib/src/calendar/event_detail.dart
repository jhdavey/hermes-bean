...
Future<void> _showCalendarEventDetails(
  ...
  BeanCalendarEvent event, {
  required List<BeanEventCategory> eventCategories,
  ...
  required Future<BeanEventCategory> Function({
  ...
  Future<void> Function(BeanCalendarEvent event, bool isCritical)?
  onCriticalChanged,
  Future<void> Function(
    BeanCalendarEvent event, {
    List<Object> deleteFromWorkspaceIds,
...
class _CalendarEventDetailPage extends StatefulWidget {
  ...
  onEventCategoryDeleted;
  final Future<void> Function(BeanCalendarEvent event, bool isCritical)?
  onCriticalChanged;
  final Future<void> Function(
    BeanCalendarEvent event, {
    List<Object> deleteFromWorkspaceIds,
...
class _CalendarEventDetailPageState extends State<_CalendarEventDetailPage> {
  ...
  late String _status;
  late List<BeanEventCategory> _categories;
  ...
  List<BeanPlaceSuggestion> _placeSuggestions = const [];
  BeanPlaceDetails? _selectedPlace;
  ...
  BeanApiClient? get _placesApiClient => _HeyBeanRuntimeServices.apiClient;
  ...
  Future<void> _selectPlaceSuggestion(BeanPlaceSuggestion suggestion) async {
    final apiClient = _placesApiClient;
    if (apiClient == null) return;
    ...
    try {
      ...
      setState(() {
        _selectedPlace = BeanPlaceDetails(
  ...
  Future<void> _openCategoryEditModal(BeanEventCategory category) async {
  ...
  List<BeanEventCategory> get _categoryChipValues {
    final byName = <String, BeanEventCategory>{};
    ...
    if (selectedName.isNotEmpty &&
      ...
      byName[selectedName.toLowerCase()] = BeanEventCategory(
        id: -1,
        name: selectedName,
        color: _color,
      );
  ...
  void _selectCategory(BeanEventCategory category) {
    setState(() {
  ...
  Future<void> _deleteCategoryValues(BeanEventCategory category) async {
    if (category.id < 0) {
      setState(() {
  ...
  void _setEventWorkspaceSelected(BeanWorkspace workspace, bool selected) {
    final value = _workspaceValue(workspace);
    setState(() {
      _validationError = null;
      if (!_creatingEvent) {
...
class _LocationSuggestionTile extends StatelessWidget {
  ...
  final BeanPlaceSuggestion suggestion;
...
class _EventCategoryChip extends StatelessWidget {
  ...
  final Key deleteKey;
  final Key editKey;
  final BeanEventCategory category;
  final Color color;
  final bool selected;
  final bool saving;
...
Color _calendarEventColor(BeanCalendarEvent event) {
  final value = event.color;
  if (value == null) return _colorFromHex(_beanGreenCategoryColor);
  return _colorFromHex(value);
}

bool _eventHasNotes(BeanCalendarEvent event) =>
    (event.notes ?? '').trim().isNotEmpty;

BeanPlaceDetails? _placeDetailsFromEvent(BeanCalendarEvent event) {
...
String _eventSubtitle(BeanCalendarEvent event) {
  final parts = <String>[
...
String _calendarEventBlockKey(BeanCalendarEvent event) {
...
String _calendarEventBlockKeyForDay(BeanCalendarEvent event, DateTime day) {
