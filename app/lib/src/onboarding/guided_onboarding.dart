...
class _GuidedBeanOnboardingScreenState
  ...
  void _submitName() {
    ...
    if (name.length < 2) {
      _setError('Enter the name you want Bean to use.');
      return;
    }
    _name = name;
  ...
  Future<void> _submitPassword() async {
    ...
    try {
      ...
      setState(() => _busy = false);
      _showStep(_GuidedOnboardingStep.personality);
    } catch (error) {
      if (!mounted) return;
      setState(() {
        _busy = false;
  ...
  void _selectPersonality(String key) {
    if (!_controlsEnabled) return;
    final matchingOptions = _agentPersonalityOptions.where(
      (option) => option.key == key,
    );
    if (matchingOptions.length != 1) {
      _setError('Choose one of the listed personality styles.');
      return;
    }
    setState(() => _personality = matchingOptions.single.key);
    _showStep(_GuidedOnboardingStep.location);
  }
    ...
    try {
      ...
      setState(() {
        ...
        _error = error is _GuidedLocationIssue
            ? error.message
            : 'Your city could not be read. Skip this step or try again.';
      });
    }
  }
  ...
  Future<void> _savePreferences() {
    ...
    if (personality == null) {
      throw StateError('A personality selection is required.');
    }
    final personalityLabel = _agentPersonalityOptions
        .singleWhere((option) => option.key == personality)
        .label;
    final context = [
  ...
  Future<String> _currentCity() async {
    ...
    if (permission == LocationPermission.denied) {
      throw const _GuidedLocationIssue(
        'Location permission was not granted. Allow location or skip this step.',
      );
    }
    if (permission == LocationPermission.deniedForever) {
      await Geolocator.openAppSettings();
      throw const _GuidedLocationIssue(
        'Location permission is blocked. App settings were opened; allow it there or skip this step.',
      );
    }
    ...
    if (!serviceEnabled) {
      ...
      throw const _GuidedLocationIssue(
        'Location services are off. Location settings were opened; enable them or skip this step.',
      );
    }
    ...
    if (placemarks.isEmpty) {
      throw const _GuidedLocationIssue(
        'The city could not be identified from this location. Skip this step or add a city later in Settings.',
      );
    }
  ...
  int get _stepNumber => switch (_step) {
    ...
    _GuidedOnboardingStep.personality => 5,
    _GuidedOnboardingStep.location => 6,
    _GuidedOnboardingStep.tourChoice || _GuidedOnboardingStep.tour => 7,
    _GuidedOnboardingStep.plan => 8,
  };

  String get _stepTitle => switch (_step) {
    _GuidedOnboardingStep.name => 'What should Bean call you?',
    ...
    _GuidedOnboardingStep.personality => 'Choose Bean’s personality',
    _GuidedOnboardingStep.location => 'Share city-level location?',
  ...
  String get _stepDescription => switch (_step) {
    ...
    _GuidedOnboardingStep.personality =>
      'Your account is ready. Check your email to verify it, then select one response style.',
    _GuidedOnboardingStep.location =>
      'Location is optional and is reduced to city-level context for weather and planning.',
    ...
    _GuidedOnboardingStep.plan =>
      'Select the subscription that fits how you plan to use Bean.',
  };
  ...
  Widget _buildStepContent() => switch (_step) {
    ...
    _GuidedOnboardingStep.personality => _GuidedPersonalityPicker(
      selected: _personality,
      enabled: _controlsEnabled,
      onSelected: _selectPersonality,
    ),
    _GuidedOnboardingStep.location => _GuidedLocationActions(
      enabled: _controlsEnabled,
