...
void main() {
  ...
  test('realtime voice cancellation phrases end the active conversation', () {
    ...
    expect(realtimeVoiceCancelForTesting('cancel it'), isTrue);
    expect(realtimeVoiceCancelForTesting('cancel my request'), isTrue);
    expect(realtimeVoiceCancelForTesting('forget that'), isTrue);
    expect(realtimeVoiceCancelForTesting('scratch that'), isTrue);
    expect(realtimeVoiceCancelForTesting('stop talking Bean'), isTrue);
    expect(realtimeVoiceCancelForTesting('pause'), isTrue);
    expect(realtimeVoiceCancelForTesting('um pause'), isTrue);
    expect(realtimeVoiceCancelForTesting('so pause'), isTrue);
    expect(realtimeVoiceCancelForTesting('hold up'), isTrue);
    expect(realtimeVoiceCancelForTesting('hold on'), isTrue);
    expect(realtimeVoiceCancelForTesting('hang on'), isTrue);
    expect(realtimeVoiceCancelForTesting('wait'), isTrue);
    expect(realtimeVoiceCancelForTesting('wait stop'), isTrue);
    expect(realtimeVoiceCancelForTesting('wait a second'), isTrue);
    expect(realtimeVoiceCancelForTesting('one second'), isTrue);
    expect(realtimeVoiceCancelForTesting('just a second'), isTrue);
    expect(realtimeVoiceCancelForTesting('give me one second'), isTrue);
    expect(realtimeVoiceCancelForTesting('give me a moment'), isTrue);
    expect(realtimeVoiceCancelForTesting('pause for a second'), isTrue);
    expect(realtimeVoiceCancelForTesting('pause for now'), isTrue);
    expect(realtimeVoiceCancelForTesting('stop right there'), isTrue);
    expect(realtimeVoiceCancelForTesting('stop for now'), isTrue);
    expect(realtimeVoiceCancelForTesting('hold that thought'), isTrue);
    expect(realtimeVoiceCancelForTesting('hang on a second'), isTrue);
    expect(realtimeVoiceCancelForTesting('let me stop you'), isTrue);
    expect(realtimeVoiceCancelForTesting('Hey Ben pause'), isTrue);
    expect(realtimeVoiceCancelForTesting('Hay Beans hold on'), isTrue);
    expect(realtimeVoiceCancelForTesting('Okay Beem cancel it'), isTrue);
    expect(realtimeVoiceCancelForTesting('Hey Bean what time is it'), isFalse);
    expect(realtimeVoiceCancelForTesting('Order coffee beans'), isFalse);
  });

  test('realtime voice distinguishes speech stop from work cancellation', () {
    expect(realtimeVoiceBackgroundWorkCancelForTesting('never mind'), isTrue);
    expect(realtimeVoiceBackgroundWorkCancelForTesting('cancel it'), isTrue);
    expect(realtimeVoiceBackgroundWorkCancelForTesting('cancel that'), isTrue);
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('Hey Bean forget it'),
      isTrue,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('stop talking Bean'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('stop listening'),
      isFalse,
    );
    expect(realtimeVoiceBackgroundWorkCancelForTesting('pause'), isFalse);
    expect(realtimeVoiceBackgroundWorkCancelForTesting('hold up'), isFalse);
    expect(realtimeVoiceBackgroundWorkCancelForTesting('hold on'), isFalse);
    expect(realtimeVoiceBackgroundWorkCancelForTesting('hang on'), isFalse);
    expect(realtimeVoiceBackgroundWorkCancelForTesting('wait'), isFalse);
    expect(realtimeVoiceBackgroundWorkCancelForTesting('wait stop'), isFalse);
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('wait a second'),
      isFalse,
    );
    expect(realtimeVoiceBackgroundWorkCancelForTesting('one second'), isFalse);
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('just a second'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('give me one second'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('give me a moment'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('pause for now'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('stop right there'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('stop for now'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('hold that thought'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('hang on a second'),
      isFalse,
    );
    expect(
      realtimeVoiceBackgroundWorkCancelForTesting('let me stop you'),
      isFalse,
    );
  });

  test('realtime voice treats polite endings as closure, not work cancel', () {
    for (final transcript in [
      ...
      expect(
        BeanRealtimeConversation.realtimePoliteConversationEndForTesting(
          transcript,
        ),
        isTrue,
        reason: transcript,
      ...
      expect(
        BeanRealtimeConversation.realtimeConversationEndForTesting(transcript),
        isTrue,
        reason: transcript,
      );
      expect(
        realtimeVoiceBackgroundWorkCancelForTesting(transcript),
        isFalse,
        reason: transcript,
      );
    }
  ...
  test('realtime wake phrase accepts common Bean transcription variants', () {
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
    ...
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
        'Hay beans what is on my calendar today',
      ),
      'what is on my calendar today',
    );
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
        'HeyBean can you hear me',
      ),
      'can you hear me',
    );
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
        'Hey Being what is next',
      ),
      'what is next',
    );
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
        'Hey Dean show my reminders',
      ),
      'show my reminders',
    );
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
        'Okay Beem plan my day',
      ),
      'plan my day',
    );
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
        'Hay Bien what reminders are due',
      ),
      'what reminders are due',
    );
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
        'Um hey Bean uh what is next',
      ),
      'what is next',
    );
    expect(
      BeanRealtimeConversation.realtimeCommandAfterWakePhraseForTesting(
        'Well hey Bean so what is next',
      ),
      'what is next',
    );
    expect(
      BeanRealtimeConversation.realtimeNormalizedVoiceCommandForTesting(
        'Hmm okay Beem um plan my day',
      ),
      'plan my day',
    );
    expect(
      BeanRealtimeConversation.realtimeNormalizedVoiceCommandForTesting(
        'So okay Beem well plan my day',
      ),
      'plan my day',
    );
    expect(
    ...
    expect(
      BeanRealtimeConversation.realtimeNormalizedVoiceCommandForTesting(
        'Hey Ben what is on my schedule today',
      ),
  ...
  test('realtime dashboard refresh ack wait stays within latency budget', () {
    final started = DateTime.utc(2026, 7, 3, 12);

    expect(
      BeanRealtimeConversation.realtimeRemainingContextRefreshBudgetForTesting(
        started,
        started.add(const Duration(milliseconds: 80)),
      ),
    ...
    expect(
      BeanRealtimeConversation.realtimeRemainingContextRefreshBudgetForTesting(
        started,
        started.add(const Duration(milliseconds: 260)),
      ),
      Duration.zero,
    );
  });

  test('realtime response payloads support low-latency interruption', () {
    expect(realtimeResponseCreatePayloadForTesting(), {
      'type': 'response.create',
      'response': {
        'modalities': ['text', 'audio'],
      },
    });
    expect(realtimeResponseCreatePayloadForTesting(audioResponse: false), {
      'type': 'response.create',
      'response': {
        'modalities': ['text'],
      },
    ...
    expect(
      realtimeResponseCreatePayloadForTesting(
        audioResponse: false,
        textResponse: false,
      ),
      {
    ...
    expect(realtimeResponseCreatePayloadForTesting(textResponse: false), {
      'type': 'response.create',
      'response': {
        'modalities': ['audio'],
      },
    });
    final freshContextPayload =
        realtimeFreshContextUnavailableItemPayloadForTesting(
          userContent: '  what is on   my calendar today ',
          reason: 'timeout',
        );
    final freshContextText =
        (freshContextPayload['item'] as Map)['content'] as List;
    final freshContextJson =
        jsonDecode((freshContextText.first as Map)['text'] as String)
            as Map<String, Object?>;

    expect(freshContextPayload['type'], 'conversation.item.create');
    expect(freshContextJson['realtime_fresh_context_unavailable'], isTrue);
    expect(freshContextJson['user_request'], 'what is on my calendar today');
    expect(freshContextJson['reason'], 'timeout');
    expect(
      freshContextJson['rules'].toString(),
      contains('content set exactly to user_request'),
    );
    expect(
      freshContextJson['rules'].toString(),
      contains('Do not summarize or alter user_request'),
    );
    expect(
      freshContextJson['rules'].toString(),
      contains('Do not answer from stale calendar'),
    );
    expect(realtimeResponseCancelPayloadForTesting('resp_123'), {
      'type': 'response.cancel',
      'response_id': 'resp_123',
    });
    expect(realtimeResponseCancelPayloadForTesting(null), {
      'type': 'response.cancel',
    });
    expect(realtimeOutputAudioBufferClearPayloadForTesting(), {
      'type': 'output_audio_buffer.clear',
    });
    expect(realtimeConversationItemTruncatePayloadForTesting('item_123', 450), {
  ...
  test('realtime latency metrics preserve voice turn timing', () {
    final started = DateTime.utc(2026, 7, 3, 12);
    final responseCreate = started.add(const Duration(milliseconds: 180));
    final firstAssistant = started.add(const Duration(milliseconds: 520));
    final completed = started.add(const Duration(milliseconds: 1250));

    expect(
      realtimeLatencyMetricsForTesting(
        turnStartedAt: started,
        responseCreateRequestedAt: responseCreate,
        firstAssistantSignalAt: firstAssistant,
  ...
  test('realtime usage latency payload only sends supported metrics', () {
    expect(
      realtimeUsageLatencyPayloadForTesting({
        'transcript_to_response_create_ms': 180,
        'response_create_to_first_assistant_ms': 340,
        'transcript_to_first_assistant_ms': 520,
        'turn_completed_ms': 1250,
  ...
  test('realtime turn persistence preserves current user content fallback', () {
    expect(
      BeanRealtimeConversation.realtimePersistedUserContentForTesting(
        pendingUserContent: '  what is next  ',
        currentUserContent: 'ignored fallback',
      ),
      'what is next',
    );
    expect(
      BeanRealtimeConversation.realtimePersistedUserContentForTesting(
        ...
        currentUserContent: '  what about tomorrow  ',
      ),
      'what about tomorrow',
    );
    expect(
  ...
  test('realtime response completion keeps original user turn content', () {
    expect(
      BeanRealtimeConversation.realtimeActiveResponseUserContentForTesting(
        activeResponseUserContent: ' what is next ',
        pendingUserContent: 'what about tomorrow',
        currentUserContent: 'ignored fallback',
      ),
      'what is next',
    );
    ...
    expect(
      BeanRealtimeConversation.realtimeActiveResponseUserContentForTesting(
        activeResponseUserContent: null,
        pendingUserContent: null,
        currentUserContent: '  move it to three  ',
      ),
      'move it to three',
    );
  });

  test('realtime barge-in cleanup preserves active follow-up turn', () {
    expect(
      BeanRealtimeConversation.realtimeInterruptedResponsePreservesTurnForTesting(
        pendingUserContent: 'what about tomorrow',
        currentUserContent: null,
      ),
      isTrue,
    ...
    expect(
      BeanRealtimeConversation.realtimeInterruptedResponsePreservesTurnForTesting(
        pendingUserContent: null,
        currentUserContent: 'make that 3 pm',
      ),
      isTrue,
    );
    expect(
  ...
  test('realtime stale interrupted response does not clear active response', () {
    expect(
      BeanRealtimeConversation.realtimeResponseDoneMatchesCurrentForTesting(
        payloadResponseId: 'resp_new',
    ...
    expect(
      BeanRealtimeConversation.realtimeMissingResponseIdConsumesInterruptedForTesting(
        payloadResponseId: null,
        hasPendingInterruptedResponseIds: true,
      ),
      isTrue,
    );
    expect(
  ...
  test('realtime stale interrupted output item does not reactivate speech', () {
    const assistantItem = {
      'type': 'message',
      'role': 'assistant',
      'id': 'item_123',
    ...
    expect(
      BeanRealtimeConversation.realtimeAssistantOutputItemIdForTesting(
        assistantItem,
        interruptedPayload: false,
      ),
      'item_123',
    );
    expect(
    ...
    expect(
      BeanRealtimeConversation.realtimeAssistantAudioDoneMatchesCurrentForTesting(
        payloadResponseId: 'resp_current',
        currentResponseId: 'resp_current',
      ),
      isTrue,
    );
    expect(
    ...
    expect(
      BeanRealtimeConversation.realtimeStatusAfterAssistantAudioDoneForTesting(
        conversationActive: true,
        transcriptionOnlyReleasePending: true,
      ),
      isNull,
    );
    expect(
    ...
    final audioDoneReadyDetails =
        BeanRealtimeConversation.realtimeAudioDoneReadyDetailsForTesting(
          responseId: ' resp_current ',
          status: 'listening',
          conversationActive: true,
          micEnabled: true,
          microphoneTrackCount: 1,
          transcriptionOnlyReleasePending: false,
          backgroundWorkActive: false,
          audioElapsedMs: 840,
        );
    expect(audioDoneReadyDetails, {
      ...
      'audio_elapsed_ms': 840,
    });
  });

  test('realtime barge-in clears interrupted internal prompt state', () {
    expect(
      BeanRealtimeConversation.realtimeBargeInInterruptsInternalPromptForTesting(
        suppressAssistant: true,
        voiceOnlyAssistant: false,
        ignoreFunctionCalls: false,
      ),
      isTrue,
    );
    expect(
  ...
  test('realtime barge-in ignores very brief unconfirmed speech blips', () {
    final startedAt = DateTime.utc(2026, 7, 3, 12);

    expect(
      BeanRealtimeConversation.realtimeShouldStartBargeInNowForTesting(
        assistantSpeaking: true,
        userSpeechActive: true,
        speechStartedAt: startedAt,
        now: startedAt.add(const Duration(milliseconds: 40)),
    ...
    expect(
      BeanRealtimeConversation.realtimeShouldStartBargeInNowForTesting(
        ...
        userSpeechActive: true,
        speechStartedAt: startedAt,
        now: startedAt.add(const Duration(milliseconds: 70)),
        transcriptConfirmed: false,
      ),
      isTrue,
    );
    expect(
      BeanRealtimeConversation.realtimeShouldStartBargeInNowForTesting(
        ...
        speechStartedAt: startedAt,
        now: startedAt.add(const Duration(milliseconds: 10)),
    ...
    expect(
      BeanRealtimeConversation.realtimeShouldStartBargeInNowForTesting(
        ...
        userSpeechActive: true,
        speechStartedAt: startedAt,
        now: startedAt.add(const Duration(milliseconds: 120)),
        transcriptConfirmed: true,
      ),
      isFalse,
    );
    expect(
      BeanRealtimeConversation.realtimeTranscriptConfirmsBargeInForTesting(
    ...
    expect(
      BeanRealtimeConversation.realtimeTranscriptConfirmsBargeInForTesting(
        'Bean HeyBean can you hear me calendar tasks reminders',
      ),
      isFalse,
    );
    expect(
      BeanRealtimeConversation.realtimeStatusAfterSpeechStoppedForTesting(
        conversationActive: true,
        assistantSpeaking: true,
      ),
      "Bean's voice",
  ...
  test('realtime barge-in telemetry marks unavailable data channel', () {
    expect(
      BeanRealtimeConversation.realtimeBargeInDispatchDetailsForTesting(
        responseId: 'resp_123',
        ...
        truncateSent: false,
        dispatchMs: 3,
        interruptedInternalPrompt: true,
  ...
  test('realtime barge-in recovery telemetry proves answered interruption', () {
    final started = DateTime.utc(2026, 7, 3, 12);
    final completed = started.add(const Duration(milliseconds: 980));

    expect(
      BeanRealtimeConversation.realtimeBargeInRecoveryDetailsForTesting(
        startedAt: started,
        completedAt: completed,
        userContent: '  what about tomorrow  ',
        assistantText: ' You have two meetings tomorrow. ',
        functionCallCount: 0,
    ...
    expect(
      ...
      containsPair('assistant_answered', false),
    );
  });

  test('realtime barge-in recovery failures are explicit', () {
    final started = DateTime.utc(2026, 7, 3, 12);
    final completed = started.add(const Duration(milliseconds: 700));

    expect(
      BeanRealtimeConversation.realtimeBargeInRecoveryFailureDetailsForTesting(
        startedAt: started,
        completedAt: completed,
        transcript: '   ',
  ...
  test('realtime voice follow-up response delay stays near instant', () {
    expect(
      realtimeVoiceResponseDelayForTesting(
    ...
    expect(
      realtimeVoiceResponseDelayForTesting(
        releasePending: false,
        content: 'move it to tomorrow and add a reminder',
      ),
      const Duration(milliseconds: 90),
    );
    expect(
      realtimeVoiceResponseDelayForTesting(
        releasePending: false,
        content: 'move it to tomorrow and',
      ),
      const Duration(milliseconds: 180),
    );
  });

  test('realtime speech before response keeps follow-up tied to pending turn', () {
    expect(
      BeanRealtimeConversation.realtimeShouldAppendTranscriptToPendingTurnForTesting(
        responseDebounceActive: true,
        pendingResponseInterruptedBySpeech: false,
        hasPendingUserContent: true,
        isWakeTurn: false,
      ),
      isTrue,
    );
    expect(
      BeanRealtimeConversation.realtimeShouldAppendTranscriptToPendingTurnForTesting(
        responseDebounceActive: false,
        pendingResponseInterruptedBySpeech: true,
        hasPendingUserContent: true,
        isWakeTurn: false,
      ),
      isTrue,
  ...
  test('realtime speech before assistant audio cancels in-flight response', () {
    expect(
      BeanRealtimeConversation.realtimeShouldCancelInFlightResponseOnSpeechStartForTesting(
        assistantSpeaking: false,
        conversationActive: true,
    ...
    expect(
      BeanRealtimeConversation.realtimeResponseCreateInFlightAfterEventForTesting(
        current: true,
    ...
    expect(
      BeanRealtimeConversation.realtimeResponseCreateInFlightAfterEventForTesting(
        current: true,
        eventType: 'response.done',
        responseDoneMatchesCurrent: false,
      ),
      isTrue,
    );
    expect(
  ...
  test('realtime response failures preserve status details for telemetry', () {
    expect(
      BeanRealtimeConversation.realtimeResponseStatusIsFailureForTesting(
        'completed',
      ),
      isFalse,
    );
    ...
    final details =
        BeanRealtimeConversation.realtimeResponseFailureDetailsForTesting(
          payload: {
            'response': {
              'id': 'resp_failed',
              'status': 'failed',
              'status_details': {
                ...
                'error': {
                  ...
                  'message': 'Realtime response timed out.',
                },
              },
            },
          },
          responseId: 'resp_failed',
          userContent: 'schedule lunch with Sam tomorrow',
          assistantText: '',
          functionCallCount: 0,
          latencyMetrics: {'turn_completed_ms': 900},
        );

    expect(details['response_id'], 'resp_failed');
    expect(details['status'], 'failed');
    expect(details['status_detail_type'], 'failed');
    expect(details['error_type'], 'server_error');
    expect(details['error_code'], 'upstream_timeout');
    expect(details['error_message'], 'Realtime response timed out.');
    expect(details['user_content'], 'schedule lunch with Sam tomorrow');
    expect(details['assistant_answered'], isFalse);
    expect(details['function_call_count'], 0);
    expect(details['turn_completed_ms'], 900);

    final dispatchDetails =
        BeanRealtimeConversation.realtimeResponseCreateDispatchFailureDetailsForTesting(
          ...
          latencyMetrics: {
            'transcript_to_response_create_ms': 120,
            'response_create_to_first_assistant_ms': 300,
            'voice_seconds': 1.2,
          },
        );

    expect(dispatchDetails['reason'], 'dispatch_error');
    expect(dispatchDetails['message'], 'data channel closed');
    expect(dispatchDetails['audio_response'], isTrue);
    expect(dispatchDetails['text_response'], isFalse);
    expect(dispatchDetails['transcript_to_response_create_ms'], 120);
    expect(dispatchDetails['response_create_to_first_assistant_ms'], 300);
    expect(dispatchDetails.containsKey('voice_seconds'), isFalse);

    final toolOutputDetails =
        BeanRealtimeConversation.realtimeToolOutputDispatchFailureDetailsForTesting(
          callId: 'call_123',
          reason: 'data_channel_unavailable',
          message: ' ',
          createResponse: true,
          latencyMetrics: {
            'transcript_to_response_create_ms': 80,
            'voice_seconds': 0.8,
          },
        );

    expect(toolOutputDetails['call_id'], 'call_123');
    expect(toolOutputDetails['reason'], 'data_channel_unavailable');
    expect(toolOutputDetails.containsKey('message'), isFalse);
    expect(toolOutputDetails['create_response'], isTrue);
    expect(toolOutputDetails['user_visible_recovery'], isTrue);
    expect(toolOutputDetails['transcript_to_response_create_ms'], 80);
    expect(toolOutputDetails.containsKey('voice_seconds'), isFalse);
  });

  test('realtime final transcript releases active user speech gate', () {
    expect(
      BeanRealtimeConversation.realtimeUserSpeechActiveAfterFinalTranscriptForTesting(
        wasActive: true,
  ...
  test('realtime non-actionable speech recovers deferred pending response', () {
    expect(
      BeanRealtimeConversation.pendingResponseRecoveryGraceForTesting(),
      const Duration(milliseconds: 320),
    );
    expect(
      BeanRealtimeConversation.realtimePendingResponseDeferredDetailsForTesting(
        userContent: 'what is next',
        responseCreateWasInFlight: true,
    ...
    expect(
      ...
      {
        'user_content': 'what is next',
        'response_create_was_in_flight': false,
        'source': 'context_refresh',
      },
    );
    final recoveryDetails =
        BeanRealtimeConversation.realtimePendingResponseRecoveryDetailsForTesting(
          deferredAt: DateTime(2026, 7, 3, 12, 0, 0),
          recoveredAt: DateTime(2026, 7, 3, 12, 0, 0, 260),
          userContent: 'what is next',
          transcript: '',
          synthetic: false,
        );
    expect(recoveryDetails['user_content'], 'what is next');
    expect(recoveryDetails['transcript'], '');
    expect(recoveryDetails['synthetic'], isFalse);
    expect(recoveryDetails['recovery_elapsed_ms'], 260);
    expect(
      BeanRealtimeConversation.realtimeShouldRecoverDeferredResponseAfterNonActionableTranscriptForTesting(
        pendingResponseInterruptedBySpeech: true,
        pendingUserContent: 'what is next',
        transcript: '',
      ),
    ...
    expect(
      BeanRealtimeConversation.realtimeShouldRecoverDeferredResponseAfterNonActionableTranscriptForTesting(
        pendingResponseInterruptedBySpeech: true,
        pendingUserContent: 'what is next',
        transcript: 'Bean HeyBean can you hear me calendar tasks reminders',
      ),
      isTrue,
    );
    expect(
    ...
    expect(
      BeanRealtimeConversation.shouldRecoverDeferredResponseAfterSpeechStopForTesting(
        pendingResponseInterruptedBySpeech: true,
        hasPendingUserContent: true,
      ),
      isTrue,
  ...
  test('realtime context refresh is limited to app-state voice turns', () {
    expect(
      BeanRealtimeConversation.realtimeContextRefreshBudgetForTesting(),
      const Duration(milliseconds: 220),
    );
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        'what is next',
      ),
      isTrue,
    );
    ...
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        'am I free after five',
      ),
      isTrue,
    );
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        'am I available after five',
      ),
      isTrue,
    );
    expect(
    ...
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        "what's on my plate this afternoon",
      ),
      isTrue,
    );
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        'what do you remember about my running shoes',
      ),
      isTrue,
    );
    expect(
    ...
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        'what did I say about Dr Chen cardio',
      ),
      isTrue,
    );
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        'what notes do I have about protein',
      ),
      isTrue,
    );
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        'what was my earlier request about Dr Chen',
      ),
      isTrue,
    );
    expect(
    ...
    for (final transcript in [
      ...
      expect(
        BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
          transcript,
          isFollowUpTurn: true,
          previousTurnNeededDashboardContext: true,
        ),
        isTrue,
        reason: transcript,
      );
      expect(
        BeanRealtimeConversation.realtimeTranscriptLooksContextualFollowUpForTesting(
          transcript,
        ),
        isTrue,
        reason: transcript,
      ...
      expect(
        BeanRealtimeConversation.realtimeContextualFollowUpKindForTesting(
          transcript,
        ),
        'reference',
        reason: transcript,
    ...
    expect(
      BeanRealtimeConversation.realtimeShouldRefreshDashboardContextBeforeResponseForTesting(
        'move that to next Friday afternoon after the planning meeting and keep the same attendees if they are available',
        isFollowUpTurn: true,
        previousTurnNeededDashboardContext: true,
      ),
      isTrue,
    ...
    expect(
      BeanRealtimeConversation.realtimeFreshContextRecoveryTelemetryForTesting(
        pendingContent: '  what is on   my calendar today ',
  ...
  test('realtime app-state fallback covers natural schedule wording', () {
    expect(
      BeanRealtimeConversation.realtimeToolFallbackTimeoutMsForTesting,
    ...
    for (final transcript in [
      ...
      'do I have any conflicts tomorrow',
      "what's open after lunch",
      'am I double booked this afternoon',
      'what should I do next',
      "what's overdue",
      'what reminders are due soon',
      'show my pending tasks',
      ...
      'what did you just save about errand updates',
      'what did I say about Dr Chen cardio',
      'what did I tell you about the school pickup plan',
      'what notes do I have about protein',
      'show my notes about launch prep',
      'find my note about travel',
      'what are my preferences for reminders',
      'what was my earlier request about Dr Chen',
      'undo that',
      'take it back',
      'revert it',
    ...
    for (final transcript in [
      ...
      'can you revert calendar events',
      'could Bean reverse a reminder change',
    ]) {
      expect(
        BeanRealtimeConversation.realtimeVoiceCommandNeedsAgentWorkForTesting(
          transcript,
    ...
    expect(
      BeanRealtimeConversation.realtimeLateToolCallDuplicatesFallbackForTesting(
        activeUserContent: 'Hey Ben what is on my schedule today',
        fallbackQueuedUserContent: 'what is on my schedule today',
      ),
      isTrue,
    );
    expect(
      BeanRealtimeConversation.realtimeLateToolCallDuplicatesFallbackForTesting(
        activeUserContent: 'what is on my schedule tomorrow',
        fallbackQueuedUserContent: 'what is on my schedule today',
      ),
      isFalse,
    );
  ...
  test('realtime transport degraded states end voice sessions', () {
    expect(
      BeanRealtimeConversation.realtimePeerConnectionStateIsDegradedForTesting(
    ...
    expect(
      BeanRealtimeConversation.realtimeIceConnectionStateIsDegradedForTesting(
  ...
  test('realtime final voice avoids reading long details aloud', () {
    final longPlan = List.generate(
      6,
      (index) => '${index + 1}. Detailed step with enough context to be noisy.',
    ).join('\n');

    expect(
      BeanRealtimeConversation.realtimeFinalVoiceForTesting(
        userContent: 'make me a workout plan',
        quickReplyText: '',
        assistantContent: longPlan,
      ),
      {
        'text': "I've written the full workout plan in chat.",
    ...
    expect(
      BeanRealtimeConversation.realtimeFinalVoiceForTesting(
        userContent: 'what is next',
        quickReplyText: '',
        assistantContent: 'Your next event is lunch at noon.',
      ),
      {'text': 'Your next event is lunch at noon.', 'suppress_final': false},
    );
    expect(
      BeanRealtimeConversation.realtimeFinalVoiceForTesting(
        userContent: 'add a task to call mom',
        quickReplyText: 'I added that task.',
        assistantContent: 'I added that task.',
      ),
      {'text': '', 'suppress_final': true},
    );
    expect(
      BeanRealtimeConversation.realtimeFinalVoiceForTesting(
        userContent: 'schedule lunch with Sam tomorrow',
        quickReplyText: "I'll schedule that now.",
        assistantContent: 'Lunch with Sam is scheduled for tomorrow at noon.',
      ),
      {
        'text': 'Lunch with Sam is scheduled for tomorrow at noon.',
        'suppress_final': false,
      },
    );
  });

  test('realtime spoken answer quality flags long voice replies', () {
    expect(
      BeanRealtimeConversation.realtimeSpokenAnswerQualityForTesting(
        'Your next event is lunch at noon.',
      ),
      {
        'spoken_character_count': 33,
    ...
    expect(
      ...
      {
        'spoken_character_count': 48,
        'spoken_sentence_count': 3,
        'spoken_brevity_violation': true,
      },
    );
  });

  test('realtime follow-up idle timeout closes only when fully idle', () {
    expect(
      BeanRealtimeConversation.realtimeFollowUpIdleTimeoutForTesting(),
      const Duration(seconds: 30),
    );
    expect(
      BeanRealtimeConversation.realtimeShouldCloseFollowUpIdleForTesting(
        conversationActive: true,
      ),
      isTrue,
    );
  ...
  test('realtime follow-up turn telemetry distinguishes continuation turns', () {
    final responseDoneAt = DateTime.utc(2026, 7, 3, 12);
    final readyAt = responseDoneAt.add(const Duration(milliseconds: 12));

    expect(
      BeanRealtimeConversation.realtimeIsFollowUpTranscriptForTesting(
        isWakeTurn: false,
        conversationActive: true,
      ),
      isTrue,
    ...
    expect(
      BeanRealtimeConversation.realtimeTurnQualityPayloadForTesting(
        isFollowUp: true,
        isContextualFollowUp: true,
        contextualFollowUpKind: 'confirmation',
      ),
      {
    ...
    expect(
      BeanRealtimeConversation.realtimeFollowUpReadyDetailsForTesting(
    ...
    for (final transcript in [
      'actually make it Tuesday at three',
      'no wait Tuesday at three',
      'no Tuesday at three',
      'no at three',
      'no make it tomorrow',
      'no tomorrow morning',
      'no tomorrow at three',
      ...
      'wait use the other one',
      'hold on not that one',
      'make it Tuesday at three',
      'change that to Friday',
      'switch it to tomorrow',
      'set it for noon',
      'not that',
      'not the second one',
      'try the other one',
  ...
  test('realtime background queue rejects premature completion claims', () {
    expect(
      BeanRealtimeConversation.realtimeSpokenAnswerAllowsBackgroundQueueForTesting(
    ...
    expect(
      BeanRealtimeConversation.realtimeSpokenAnswerShouldSkipBackgroundQueueForTesting(
        'add a task to call mom tomorrow',
        'I’ll add that now.',
      ),
      isFalse,
    ...
    expect(
      BeanRealtimeConversation.realtimeBackgroundQuickReplyContextForTesting(
        'Done, I’ll add that now.',
      ),
      '',
    );
  ...
  test('realtime background failures speak an honest failure response', () {
    final failureVoice =
        BeanRealtimeConversation.realtimeBackgroundFailureVoiceForTesting(
          'schedule lunch with Sam tomorrow',
        );

    expect(
      failureVoice,
      "I couldn't complete that request. Please try again or type it in chat.",
    );
    expect(
      BeanRealtimeConversation.realtimeBackgroundFailureVoiceAcknowledgesFailureForTesting(
        failureVoice,
      ),
      isTrue,
    );
    expect(
      BeanRealtimeConversation.realtimeBackgroundFailureVoiceAcknowledgesFailureForTesting(
        'I am checking the latest app state now.',
      ),
      isFalse,
    );
    expect(
      BeanRealtimeConversation.realtimeBackgroundResultInstructionForTesting(
        'failed',
      ),
      contains('Do not imply success or ongoing work'),
    );
    expect(
      BeanRealtimeConversation.realtimeBackgroundResultInstructionForTesting(
        'completed',
      ),
      contains('completed result'),
    );
    expect(
      BeanRealtimeConversation.realtimeBackgroundResultInstructionForTesting(
        'completed',
      ),
      isNot(contains('verified result')),
    );
    expect(
      BeanRealtimeConversation.realtimeBackgroundResultInstructionForTesting(
        'completed',
        priorSpokenClaim: 'Your calendar is clear tomorrow.',
      ),
      allOf(
        contains('verified result'),
        contains('correct it directly'),
        contains('one short sentence'),
      ),
    );
  });

  test('realtime background watch failures preserve recovery telemetry', () {
    final failureVoice =
        BeanRealtimeConversation.realtimeBackgroundFailureVoiceForTesting(
          'schedule lunch with Sam tomorrow',
        );
    final details =
        BeanRealtimeConversation.realtimeBackgroundWatchFailureDetailsForTesting(
          runId: 42,
          userContent: 'schedule lunch with Sam tomorrow',
          attempt: 8,
          reason: 'poll_error',
    ...
    expect(details['run_id'], 42);
    expect(details['attempt'], 8);
    expect(details['reason'], 'poll_error');
    expect(details['message'], 'socket closed');
    expect(details['user_content'], 'schedule lunch with Sam tomorrow');
    expect(details['failure_voice_text'], failureVoice);
    expect(details['failure_voice_acknowledged'], isTrue);
    expect(details['failure_voice_delivered'], isTrue);
    ...
    expect(staleDetails['reason'], 'max_poll_attempts');
    expect(staleDetails['status'], 'running');
    expect(staleDetails['attempt'], 45);
    expect(staleDetails['failure_voice_acknowledged'], isTrue);
    expect(staleDetails['failure_voice_delivered'], isFalse);
  });

  test('realtime background completion telemetry distinguishes closed voice', () {
    expect(
      BeanRealtimeConversation.realtimeBackgroundCompletionEventTypeForTesting(
        deliveredVoice: true,
      ),
    ...
    expect(
      BeanRealtimeConversation.realtimeBackgroundCancelEventTypeForTesting(
        deliveredVoice: true,
      ),
  ...
  test('realtime background progress skips while assistant is busy', () {
    final details =
        BeanRealtimeConversation.realtimeBackgroundProgressSkipDetailsForTesting(
          elapsedMs: 8000,
          instruction: 'Give one brief update.',
          assistantSpeaking: true,
          responseCreateInFlight: false,
        );

    expect(details['reason'], 'assistant_busy');
    expect(details['elapsed_ms'], 8000);
    expect(details['instruction'], 'Give one brief update.');
    expect(details['assistant_speaking'], isTrue);
    expect(details['response_create_in_flight'], isFalse);
  });

  test('realtime progress spoken telemetry captures delivered speech', () {
    final details =
        BeanRealtimeConversation.realtimeProgressPromptSpokenDetailsForTesting(
          responseId: 'resp_123',
          elapsedMs: 8000,
          instruction: 'Give one brief update.',
          userRequest: 'schedule lunch with Sam tomorrow',
          alreadySpoken: ['I am checking that now.', '  ', 'Still checking.'],
          spokenText: 'Still working on that for you.',
        );

    expect(details['response_id'], 'resp_123');
    expect(details['elapsed_ms'], 8000);
    expect(details['instruction'], 'Give one brief update.');
    expect(details['user_request'], 'schedule lunch with Sam tomorrow');
    expect(details['already_spoken'], [
      'I am checking that now.',
      'Still checking.',
    ]);
    expect(details['spoken_text'], 'Still working on that for you.');
    expect(details['spoken_character_count'], 30);
    expect(details['spoken_sentence_count'], 1);
  });

  test('realtime background completion defers while voice turn is busy', () {
    expect(
      BeanRealtimeConversation.pendingBackgroundDeliveryGraceForTesting(),
      const Duration(milliseconds: 260),
    );
    expect(
      BeanRealtimeConversation.realtimeBackgroundResultDeliveryBusyForTesting(
        assistantSpeaking: true,
        responseCreateInFlight: false,
      ),
      isTrue,
    ...
    const spokenText = 'Lunch with Sam is scheduled for tomorrow at noon.';
    final details =
        BeanRealtimeConversation.realtimeBackgroundDeliveryDeferredDetailsForTesting(
          runId: 123,
    ...
    expect(details['reason'], 'assistant_busy');
    expect(details['run_id'], 123);
    expect(details['spoken_character_count'], spokenText.length);
    expect(details['spoken_text'], spokenText);
    expect(details['assistant_speaking'], isTrue);
    expect(details['response_create_in_flight'], isFalse);
    expect(details['user_speech_active'], isFalse);

    final userSpeakingDetails =
        BeanRealtimeConversation.realtimeBackgroundDeliveryDeferredDetailsForTesting(
          runId: 124,
          spokenText: spokenText,
          priorSpokenClaim: '  Your calendar   is clear tomorrow. ',
    ...
    expect(userSpeakingDetails['reason'], 'user_speaking');
    expect(userSpeakingDetails['run_id'], 124);
    expect(userSpeakingDetails['verification_repair'], isTrue);
    expect(
      userSpeakingDetails['prior_spoken_claim'],
      'Your calendar is clear tomorrow.',
    );
    expect(userSpeakingDetails['user_speech_active'], isTrue);

    expect(
      BeanRealtimeConversation.shouldDeliverPendingBackgroundCompletionAfterSpeechStopForTesting(
        hasPendingCompletion: true,
        backgroundDeliveryBusy: false,
      ),
      isTrue,
    );
    expect(
  ...
  test('realtime flags unsupported direct answers that should use fresh data', () {
    final backgroundRequired =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'schedule lunch with Sam tomorrow',
          assistantText: 'I scheduled that for tomorrow.',
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final missingFreshContext =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'what is on my calendar today',
          assistantText: 'You have lunch at noon.',
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final guessedAvailability =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'am I free after five',
          assistantText: "You're free after five.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final guessedEmptySchedule =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'do I have meetings tomorrow',
          assistantText: 'No meetings tomorrow.',
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final guessedNextMeeting =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'what is my next meeting',
          assistantText: 'Your next meeting is with Sam.',
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final guessedClearCalendar =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'what is on my calendar tomorrow',
          assistantText: 'Your calendar is clear tomorrow.',
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final guessedNoTasks =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'what tasks are due today',
          assistantText: "You don't have any tasks due today.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final foundLunch =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'what is on my calendar today',
          assistantText: 'I found lunch at noon.',
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final sawNoConflicts =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'do I have any conflicts tomorrow',
          assistantText: 'I see no conflicts tomorrow.',
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final theresLunch =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'what is on my schedule today',
          assistantText: "There's lunch at noon.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final warmedCurrentWeather =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: "what's the weather right now",
          assistantText: "It's 82 degrees and partly cloudy.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final warmedNearMeWeather =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: "what's the weather near me right now",
          assistantText: "It's 82 degrees and partly cloudy.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final warmedMyAreaWeather =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: "what's the weather in my area",
          assistantText: "It's 82 degrees and partly cloudy.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final warmedHereWeather =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: "what's the weather around here",
          assistantText: "It's 82 degrees and partly cloudy.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final forecastDirectAnswer =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: 'what is the weather tomorrow',
          assistantText: "It'll be sunny tomorrow.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    final namedLocationCurrentWeather =
        BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
          userTranscript: "what's the weather in Orlando right now",
          assistantText: "It's 82 degrees and partly cloudy.",
          hasFunctionCall: false,
          contextRefreshSucceeded: false,
        );
    expect(backgroundRequired, containsPair('reason', 'background_required'));
    expect(
      missingFreshContext,
      containsPair('reason', 'missing_fresh_context'),
    );
    expect(
      guessedAvailability,
      containsPair('reason', 'missing_fresh_context'),
    ...
    expect(
      guessedEmptySchedule,
      containsPair('reason', 'missing_fresh_context'),
    );
    expect(guessedNextMeeting, containsPair('reason', 'missing_fresh_context'));
    expect(
      guessedClearCalendar,
      containsPair('reason', 'missing_fresh_context'),
    );
    expect(guessedNoTasks, containsPair('reason', 'missing_fresh_context'));
    expect(foundLunch, containsPair('reason', 'missing_fresh_context'));
    expect(sawNoConflicts, containsPair('reason', 'missing_fresh_context'));
    expect(
      BeanRealtimeConversation.realtimeUnsupportedDirectAnswerNeedsBackgroundVerificationForTesting(
        theresLunch,
      ),
      isTrue,
    );
    expect(warmedCurrentWeather, isNull);
    expect(warmedNearMeWeather, isNull);
    expect(warmedMyAreaWeather, isNull);
    expect(warmedHereWeather, isNull);
    expect(forecastDirectAnswer, containsPair('reason', 'background_required'));
    expect(
      namedLocationCurrentWeather,
      containsPair('reason', 'background_required'),
    );
    expect(
      BeanRealtimeConversation.realtimeUnsupportedDirectAnswerNeedsBackgroundVerificationForTesting(
        backgroundRequired,
    ...
    expect(
      BeanRealtimeConversation.realtimeUnsupportedDirectAnswerForTesting(
        userTranscript: 'what is the weather tomorrow',
        assistantText: "I'll check that now.",
        hasFunctionCall: false,
        contextRefreshSucceeded: false,
      ),
      isNull,
