...
class LiveKitManager {
  ...
  ///
  /// On iOS the LiveKit room can report connected and the microphone track can
  /// be published a few frames before the reliable data channel is open. The
  /// ElevenLabs startup override is sent immediately after room-ready, so retry
  /// the publish briefly instead of failing the whole voice session with
  /// dataChannelGetBufferedAmountFailed.
  Future<void> sendMessage(Map<String, dynamic> message) async {
    ...
    for (var attempt = 0; attempt < 8; attempt += 1) {
      try {
        ...
        final dataChannelNotReady =
            message.contains('dataChannelGetBufferedAmountFailed') ||
                message.contains('dataChannel not found or not opened');
        if (!dataChannelNotReady || attempt == 7) {
          break;
        }
    ...
    final error = Exception('Failed to send message: $lastError');
