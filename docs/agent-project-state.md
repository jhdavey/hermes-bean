...
- Public landing Bean onboarding is single-path tap-to-wake: visitors click/tap the centered hero Bean above the feature icons. The public top bar uses a Bean icon + `Bean` brand at the left, centered nav links, no nav CTA, and `Login` on the far right. The hero Bean control is icon-only with no circular listening/ring indicator; `Tap to wake up` and `Volume on · allow mic` sit below as simple grey helper text, and the horizontal hero tool row only shows Calendar, Tasks, Notes, and Reminders. There is no handwritten cue or arrow. While the landing page is at the hero top, Bean stays centered above the hero icons; as the visitor starts scrolling, the landing Bean travels upward with its original page position, then only after it reaches the top dock does it shrink into the compact top-centered presence and fade in the full-width white backdrop so page content scrolls behind it cleanly. The public early-access bar is static at the top of the page, not sticky viewport chrome. Public/register Bean has inline critical sizing plus explicit image dimensions so the logo never paints at its natural 790px size before bundled CSS loads; landing-to-register voice navigation also stores `heybean.publicBean.handoff` so the same fixed top-centered Bean position survives the page/view load. Public landing CTAs say `Try it for free`; the hero headline is `Hi! I'm Bean. Your new assistant!`. Public Bean starts the fast ElevenLabs Landing Guide, first says `Hey, I'm Bean, can you hear me?`, then answers questions or gives a short conversational tour. A plain hearing-check confirmation such as “yes” should only get `Great — I’m Bean. I can give you a quick tour or answer questions.` and must not move/scroll the page. Bean should not push signup while answering pricing/features; it only opens `/register?from=bean` when the visitor explicitly agrees to start or try signup. When visitors ask to start signup, Bean says exactly `Ok, i'll just get some quick info from you and show you around`, then navigates to Bean onboarding after a short delay. Non-voice CTAs use their own attribution (`from=hero_cta`, `final_cta`, `beta_banner`, etc.) and the register API preserves this `source` on early-access signup rows.
