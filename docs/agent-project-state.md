...
Last updated: 2026-07-24T12:57:00Z
...
- Production Zero Chrome behavior to verify/deploy: this commit (`Revamp Bean onboarding zero chrome`)
...
- Signup voice context remains `signup_onboarding` with the `showSignupInput` client tool. The public signup Bean dispatches safe `bean:signup-progress` events into the active ElevenLabs conversation for name → theme → email → password → account/waitlist/dashboard transitions. For admitted users, the same public signup Bean remains visually present through dashboard preview tour and first-action selection; the normal authenticated dashboard Bean is suppressed during that connected onboarding conversation and the public signup Bean is removed when deferred paywall/subscription flow opens. Waitlist/capacity still stops before dashboard tour/paywall.
