...
- Public landing Bean onboarding is single-path tap-to-talk: visitors click/tap the fixed top-left Bean control (`Tap to talk`) rather than choosing preset options or entering an email on the landing page. Public Bean starts the fast ElevenLabs Landing Guide, first says `Hey, I'm Bean, can you hear me?`, then answers questions, gives a short conversational tour, or opens `/register?from=bean` when the visitor agrees to start signup. The experience should feel like the same Bean continuing into hard-coded app onboarding; public Bean never collects passwords or payment details by voice.
