...
Last updated: 2026-07-24T00:07:00Z
...
- Main branch behavior at last update: `1d91bede Elevate Bean signup onboarding polish` plus this doc update.
- Local working tree at last update was clean after the world-class signup onboarding polish deploy.
- Production host/path: `forge@heybean.org:/home/forge/heybean.org/current`
- Production commit last verified in prior work: `1d91bedeff1716aa9b5de7ddc19955dca257f4b3`
...
- Public landing Bean onboarding is single-path tap-to-talk: visitors click/tap the fixed top-left Bean control (`Tap to talk`) with the handwritten cue `Hey! I'm over here!`; they do not choose preset options or enter an email on the landing page. Public Bean starts the fast ElevenLabs Landing Guide, first says `Hey, I'm Bean, can you hear me?`, then answers questions, gives a short conversational tour, or opens `/register?from=bean` when the visitor agrees to start signup. Non-voice CTAs use their own attribution (`from=topbar_button`, `hero_cta`, `final_cta`, `beta_banner`, etc.) and the register API preserves this `source` on early-access signup rows. The same tap-to-talk Bean control is present on `/register` with `signup_onboarding` context, but the handwritten cue/arrow only appears in the public nav/landing chrome, not inside signup. Register uses a centered Bean surface that matches the landing-page style without decorative background circles/blobs: a compact register-only early-access pill replaces the old full-width signup CTA banner, Bean button stays top-center, output sits below with calmer mobile-first type scale, compact old bubbles fade upward, and the hovering input/send composer sits directly below the current Bean output. Signup secondary exits are quiet `Log in · Use form instead` utility links, and the conversation trail uses privacy-safe labels such as `Name added` and `Email added` instead of replaying typed personal data. Signup tap-to-talk shows the mic/volume helper during startup/listening and overrides the first spoken message with a signup guide intro that tells users to keep volume/mic on and type answers into the input. As the deterministic signup UI advances, it dispatches safe `bean:signup-progress` events into the active public ElevenLabs conversation (`sendUserMessage`) so Bean keeps talking through name → theme → email → password → account/waitlist/dashboard transitions; progress prompts never include typed names, emails, passwords, or payment details. Dark mode uses a plain full-scene landing-aligned background and themed Bean/control/composer surfaces, not ornamental radial blobs and not only text/input color changes. Bean tells users to type name/email/password into the input and press Send, and tapping Bean mutes/ends voice so the user can continue by text. The `/register?from=bean` app onboarding intro omits the repeated `Hi, I’m Bean.` and asks `What is your first and last name?`; normal `/register` keeps `Hi, I’m Bean.` The register SPA removes the public signup Bean control when it transitions to waitlist, dashboard preview, or subscription states, so it does not linger over the authenticated dashboard. Public signup errors are sanitized so raw SQL/internal runtime details are not exposed. The waitlist stop stays after account creation/name/theme/email/password. Admitted users see the dashboard tour and first-action choice/skip before the subscription paywall; the backend product API gate remains subscription-required until Stripe trial/active status. Public/onboarding Bean never collects passwords or payment details by voice.
