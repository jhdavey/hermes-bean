...
- A classifier rejection returns silently to wake-only operation. A strict `Hey Bean` proposal is compatible with either learned addressed-to-Bean positive class because the stronger local proposal already supplies the `Hey` evidence; either positive is evaluated at the strict proposal operating threshold, and deterministic code preserves the strict activation and its safe boundary. An address-only proposal remains compatible only with `missed_hey_confirmation` at the address operating threshold. A positive learned class and a compatible proposal are both required before deterministic code may use the proposal timestamp. There is no direct-KWS acceptance, separate address model, base/guard model, phrase fallback, or parallel verifier.
